...
function producedPerCycle(type: "CHICKEN" | "SHEEP" | "COW", level: number) {
...
function pointsPerCycle(type: "CHICKEN" | "SHEEP" | "COW", level: number) {
  if (level < 4) return 0;

  if (type === "CHICKEN") return level >= 5 ? 3 : 2;
  if (type === "SHEEP") return level >= 5 ? 6 : 4;
  if (type === "COW") return level >= 5 ? 10 : 7;
