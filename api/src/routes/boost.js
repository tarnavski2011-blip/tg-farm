...
router.post("/activate", async (req, res) => {
    ...
    if (user.diamonds < BOOST_PRICE_DIAMONDS) {
        return res.status(400).json({ error: "Not enough diamonds", need: BOOST_PRICE_DIAMONDS, have: user.diamonds });
    }
    const now = Date.now();
    const base = user.boostUntil && user.boostUntil.getTime() > now ? user.boostUntil.getTime() : now;
    ...
    return res.json({ ok: true, diamonds: updated.diamonds, boostUntil: updated.boostUntil });
});
router.post("/vip", async (req, res) => {
    ...
    if (user.diamonds < VIP_PRICE_DIAMONDS) {
        return res.status(400).json({ error: "Not enough diamonds", need: VIP_PRICE_DIAMONDS, have: user.diamonds });
    }
    const now = Date.now();
    ...
    return res.json({ ok: true, diamonds: updated.diamonds, vipUntil: updated.vipUntil });
});
